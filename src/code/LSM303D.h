...
////////////////////////////////////
// The following code ist based on https://wiki.seeedstudio.com/Grove-6-Axis_AccelerometerAndCompass_V2.0/
...
void loop()
{
    ...
    for (int i=0; i<3; i++)
    {
        ...
        Spine.write(connector, i, realAccel[i]);
    }
    ...
    if(Lsm303d.isMagReady())
    {
        ...
        for (int i=0; i<3; i++)
        {
            realMag[i] = mag[i] / pow(2, 15) * 1.3;  // calculate real acceleration values, in units of Gauss
            Spine.write(connector, i + 3, realMag[i]);

        }
        heading = Lsm303d.getHeading(mag);
        tiltHeading = Lsm303d.getTiltHeading(mag, realAccel);
        Spine.write(connector, 6, tiltHeading);
    }
